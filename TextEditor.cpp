...
namespace ImGuiColorTextEdit {
	...
	void TextEditor::Render(bool aParentIsFocused)
	{
		...
		if (scrollY != _lastScrollY || contentSize.y != _lastContentSize.y)
			mNeedsWindowColorization = true;

		int lineMin = mState.mLineMin = (int)floor(scrollY / mCharAdvance.y);
		auto globalLineMax = (int)mLines.size();
		int lineMax = mState.mLineMax = std::max(0, std::min((int)mLines.size() - 1, mState.mLineMin + (int)floor((scrollY + contentSize.y) / mCharAdvance.y)));
		...
		if (!mLines.empty())
		{
			...
			{
				ImVec2 lineStartScreenPos = ImVec2(cursorScreenPos.x, cursorScreenPos.y + lineMin * mCharAdvance.y);
				ImVec2 textScreenPos = ImVec2(lineStartScreenPos.x + mTextStart, lineStartScreenPos.y);

				auto& line = mLines[lineMin];
				longest = std::max(mTextStart + TextDistanceToLineStart(Coordinates(lineMin, GetLineMaxColumn(lineMin))), longest);
				auto columnNo = 0;
				Coordinates lineStartCoord(lineMin, 0);
				Coordinates lineEndCoord(lineMin, GetLineMaxColumn(lineMin));
				...
				for (int c = 0; c <= mState.mCurrentCursor; c++)
				{
					...
					if (mState.mCursors[c].mSelectionEnd.mLine > lineMin)
				...
				if (mBreakpoints.count(lineMin + 1) != 0)
				{
				...
				// Draw error markers
				auto errorIt = mErrorMarkers.find(lineMin + 1);
				...
				for (int c = 0; c <= mState.mCurrentCursor; c++)
				{
					if (mState.mCursors[c].mCursorPosition.mLine == lineMin)
	...
	bool TextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
	{
		...
		if (mNeedsWindowColorization) {
			Colorize();
			mNeedsWindowColorization = false;
		}
	...
	{
		int fromLine = aFromLine == -1 ? mState.mLineMin : aFromLine;
		int toLine = aLines == -1 ? std::min((int)mLines.size(), mState.mLineMax) : std::min((int)mLines.size(), fromLine + aLines);
		mColorRangeMin = std::max(0, fromLine);
		mColorRangeMax = std::max(mColorRangeMin, toLine);
		mCheckComments = true;
	}
	...
	void TextEditor::ColorizeInternal()
	{
		...
		if (mCheckComments)
		{
			...
			auto firstChar = true;			// there are no other non-whitespace characters in the line before
